...
pub fn execute(
    ...
    const SegfaultHandling = struct {
        //Should implement setjmp/longjump in inline asm instead of linking to libc
        const jump_buf = extern struct { a: c_int, b: c_int, c: c_int, d: c_int, e: c_int, f: c_int };

        pub extern "c" fn setjmp(buf: *jump_buf) callconv(.C) bool;

        extern "c" fn longjmp(buf: *jump_buf, _: bool) callconv(.C) void;

        pub var segfault_jump_buffer: jump_buf = undefined;

        fn segfaultHandler(_: c_int, signal_info: *const std.os.siginfo_t, _: ?*const anyopaque) callconv(.C) void {
            _ = signal_info;

            longjmp(&segfault_jump_buffer, true);
    ...
    if (config.handle_traps and SegfaultHandling.setjmp(&SegfaultHandling.segfault_jump_buffer)) {
    ...
    if (config.handle_traps) std.os.sigaction(
        std.os.SIG.SEGV,
        &.{ .handler = .{ .sigaction = &SegfaultHandling.segfaultHandler }, .flags = undefined, .mask = undefined },
        &old_sigsegv_action,
    ) catch unreachable;
    defer if (config.handle_traps) std.os.sigaction(std.os.SIG.SEGV, &old_sigsegv_action, null) catch unreachable;
